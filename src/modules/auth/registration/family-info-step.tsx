...
interface FamilyInfoStepProps {
  defaultValues?: Partial<FamilyInfoSchemaType>;
  onNext: (data: FamilyInfoSchemaType) => void;
  onBack: () => void;
}
...
export default function FamilyInfoStep({
  ...
  onNext,
  onBack,
}: Readonly<FamilyInfoStepProps>) {
  const dv = defaultValues?.familyInformation;
  const {
    register,
    handleSubmit,
    control,
    ...
    defaultValues: {
      familyInformation: {
        ...
        familyLocationSameAsProfile: dv?.familyLocationSameAsProfile ?? true,
  ...
  const sameAsProfile = watch(
    "familyInformation.familyLocationSameAsProfile",
  );
